...
df = pd.read_csv(path)
print(df.head())
tempList = ['INCOME','HOME_VAL','BLUEBOOK','OLDCLAIM','CLM_AMT']
for i in tempList:
...
# X_train['AGE','CAR_AGE','INCOME','HOME_VAL'].apply(lambda x: x.fillna(x.mean()), inplace=True)

# X_test['AGE','CAR_AGE','INCOME','HOME_VAL'].apply(lambda x: x.fillna(x.mean()), inplace=True)
...
tempList = ['AGE','CAR_AGE','INCOME','HOME_VAL']
for i in tempList:
